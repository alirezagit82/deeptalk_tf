#import inspect, torch, torch_scatter
...
def scatter_(name, src, index, dim_size=None):
    ...
    if dim_size is None:
        dim_size = tf.reduce_max(index)+1

    if name == 'add':
        out = tf.math.unsorted_segment_sum(src, index, dim_size)
    elif name == 'mean':
        out = tf.math.unsorted_segment_mean(src, index, dim_size)
    elif name == 'max':
        out = tf.math.unsorted_segment_max(src, index, dim_size)
